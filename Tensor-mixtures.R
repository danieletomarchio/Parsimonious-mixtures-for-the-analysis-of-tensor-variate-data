...
TensMixtPars_init <- function(X, k, mod.list = "all", nstartR = 50, nThreads = 1, density = "TVN") {
  ...
  for (g in 1:length(k)) {
    print(paste(paste("Initializing Parsimonious Tensor-variate", density), paste("mixtures with k =", k[g])))
...
TensMixtPars_fit <- function(X, init.par = NULL, tol = 0.001, maxit = 500, nThreads = 1) {
  ...
  for (g in 1:length(k)) {
    ...
    print(paste(paste("Fitting Parsimonious Tensor-variate", density), paste("mixtures with k =", k[g])))
